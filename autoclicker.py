...
def on_mouse_click(x, y, button, pressed):
    global recording, macro_recording_target
    ...
    if macro_recording_target == 'hotkey' and pressed and button != Button.left:
        ...
        return
    if isinstance(macro_recording_target, str) and macro_recording_target.startswith('step_') and pressed and button != Button.left:
        root.after(0, lambda b=button: _step_record_cb_mouse(b))
        return
...
def set_holding(state):
    ...
    if state:
        ...
        toggle_status.config(text="🟢 ЛКМ зажата", fg="#4ade80")
    else:
        mouse_ctrl.release(Button.left)
        toggle_status.config(text="🔴 ЛКМ отпущена", fg="#f87171")
...
def set_toggle_hotkey_mouse(button):
    ...
    toggle_record_btn.config(text="Назначить клавишу", bg="#1e40af", state="normal")
    root.unbind("<KeyPress>")

def start_toggle_recording():
    global recording
    recording = True
    toggle_record_btn.config(text="Нажми любую клавишу/кнопку...", bg="#1e3a5f", state="disabled")
...
_step_record_cb_mouse = None
_step_record_cb_key = None
...
def open_step_editor(index):
    global macro_recording_target, _step_record_cb_mouse, _step_record_cb_key
    ...
    btn_entry = tk.Entry(btn_frame, textvariable=btn_var, font=lf,
                         bg="#1e293b", fg="white", insertbackground="white", width=18)
...
def show_instructions():
    ...
    text = tk.Text(win, font=lf, bg="#0f172a", fg="#94a3b8", relief="flat",
                   wrap="word", padx=20, pady=10, height=18)
    text.pack(fill='both', expand=True, padx=10)

    instructions = """🎮 ВКЛАДКА «ПЕРЕКЛЮЧАТЕЛЬ»
Устал зажимать ЛКМ во время рыбалки или гринда?
Нажми «Назначить клавишу» → выбери любую клавишу или кнопку мыши (MOUSE3, MOUSE4, MOUSE5 — всё работает).
Одно нажатие — ЛКМ зажата. Ещё раз — отпущена.
Твой палец в полном ахуе от таких регламентов? Теперь нет.
...
Каждый шаг имеет:
  → Длительность зажатия (мс)
  → Задержка после шага (мс)

Повторений: 0 = бесконечный цикл, любое число = столько раз.

💾 СОХРАНЕНИЕ И ЗАГРУЗКА
Макрос сохраняется в .json файл — можно хранить разные профили под разные игры и загружать в один клик.

🔥 Специально для тех кто заебался от тупого гринда скилов, чтобы мышь, клава и особенно палец не страдали от зажатия кнопок по регламентам игр вроде Ashes and Blood.
...
def on_close():
    if holding: mouse_ctrl.release(Button.left)
    for l in [kb_listener, ms_listener]:
        if l:
            try: l.stop()
            except: pass
...
key_f    = tkfont.Font(family="Consolas", size=14, weight="bold")
...
tk.Label(toggle_page, text="Нажми клавишу один раз — ЛКМ зажата.\nЕщё раз — отпущена.",
         font=label_f, bg="#0f172a", fg="#94a3b8", justify="center").pack(pady=(12,16))
tk.Label(toggle_page, text="Текущая клавиша", font=label_f, bg="#0f172a", fg="#64748b").pack()
toggle_hotkey_label = tk.Label(toggle_page, text="—", font=key_f, bg="#0f172a", fg="#38bdf8")
toggle_hotkey_label.pack(pady=(2,14))
toggle_record_btn = tk.Button(toggle_page, text="Назначить клавишу", font=label_f,
                              bg="#1e40af", fg="white", activebackground="#1e3a8a",
                              relief="flat", padx=16, pady=8, cursor="hand2",
                              command=start_toggle_recording)
toggle_record_btn.pack(pady=(0,24))
toggle_status = tk.Label(toggle_page, text="🔴 ЛКМ отпущена", font=status_f,
                         bg="#0f172a", fg="#f87171")
toggle_status.pack()
...
tk.Button(sl_row, text="💾 Сохранить макрос", font=label_f, bg="#0f4c81", fg="white",
          relief="flat", padx=10, pady=4, cursor="hand2",
          command=save_macro).pack(side='left')
tk.Button(sl_row, text="📂 Загрузить макрос", font=label_f, bg="#1e293b", fg="#38bdf8",
          relief="flat", padx=10, pady=4, cursor="hand2",
