...
def capture_target_key(event):
    ...
    # Try special map first
    mapped = map_keysym(key_name)
    if mapped:
        ...
        hold_target = ('key', _kb.KeyCode.from_char(key_name.lower()))
        hold_target_display = key_name.upper()
        toggle_target_label.config(text=f"[ {key_name.upper()} ]")
    else:
        # Try as pynput Key attribute
        import pynput.keyboard as _kb
        k = getattr(_kb.Key, key_name.lower(), None)
        if k:
            hold_target = ('key', k)
            hold_target_display = key_name
            toggle_target_label.config(text=f"[ {key_name} ]")
        else:
            toggle_target_label.config(text="[ ? ]")
    toggle_target_btn.config(text="Назначить", bg="#7c3aed", state="normal")
    root.unbind("<KeyPress>")
